<template>
  <div>
    ...
    <el-card>
      ...
      <el-table :data="olderList" border stripe>
        ...
        <el-table-column label="操作" align="center" v-if="this.token === '1' || this.token === '4'">
          <template v-slot ="scope">
            ...
            <el-button type="primary"  size="mini" @click="selectinter(scope.row.id,scope.row.name)">查看
            </el-button>
            <!--删除-->
      ...
      <!--      健康信息展示 -->
      <el-dialog title="健康信息" :visible.sync="healthDialogVisible" width="90%">
        <template>
          <el-table
            :data="healData"
            height="500"
        ...
        <el-card style="height: 100%">
          <el-row>
            <el-col style="width: 50%;height: 300px;">
              <label>老人姓名：</label>
              <el-select placeholder="请选择老人" v-model="queryInfos.olderId"
      ...
      <el-dialog title="修改老人信息" :visible.sync="editDialogVisible" width="60%" @close="editClosed">
        <el-form :model="editOlderForm" :rules="addFormRules" ref="editFormRef" label-width="160px">
          ...
          <el-form-item label="密码" prop = "password">
            <el-input v-model="editOlderForm.password" :disabled="isDisabled"></el-input>
          </el-form-item>
      ...
      <el-dialog title="添加老人信息" :visible.sync="insertOlder" width="60%" @close="insertClosed">
        <el-form :model="addOlderForm" :rules="addFormRules" ref="addFormRef" label-width="160px">
          ...
          <el-form-item label="密码：" prop = "password">
            <el-input v-model="addOlderForm.password"></el-input>
          </el-form-item>
          <el-form-item label="所属小区名称：" prop="communityCd">
...
export default {
  ...
  methods:{
    ...
    async getOlderList(){
      ...
      this.olderList = res;
      console.log(res)
      this.total = this.olderList.total;
    ...
    editOlderInfo(){
      this.$refs.editFormRef.validate(async valid =>{
        if(!valid) return;
        // console.log(this.editOlderForm.id)
    ...
    selectinter(id,name){
      // this.getInternalList(id)
      this.getInternalform(id,name)
    ...
    //   this.$http
    //     .post("http://localhost:9001/img/upload", formData,{
    ...
    async getInternalform(id,name){
      this.queryInfo.olderId=id
      const {data: res} = await this.$http.get("/healthy/internalList?olderId=" + this.queryInfo.olderId)//获取体征信息
      console.log(res)
      this.healData=res
      if(this.healData.istate=="异常"){

      }
    },

    //获取内科信息
    async getInternalList(id) {
      this.dataX = [];
      this.dataY = [];
      this.dataHeightY = [];
      this.dataLowY = [];
      ...
      for (let i = 0; i < this.internalList.length; i++) {
        console.log("internallist=="+this.internalList[i].temperature+this.internalList[i].createTime)
        this.dataX.push(this.$moment(this.internalList[i].createTime).format('YYYY-MM-DD'))
        this.dataY.push(this.internalList[i].heartRate)
        
        this.dataHeightY.push(this.internalList[i].bloodOxygen.split("/")[0])
        this.dataLowY.push(this.internalList[i].bloodOxygen.split("/")[1])
        this.dataFatY.push(this.internalList[i].bloodFat)
        this.dataSugarY.push(this.internalList[i].temperature)
      }
      this.myLine();
      this.myEcharts();
      this.drawLine();
