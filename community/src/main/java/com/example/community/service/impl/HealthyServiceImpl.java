...
//        healthy.setCreateTime(new SimpleDateFormat("yyyy-MM-dd").parse(healthy.getTime()));
    ...
    public List<Internal> getInternalList(Internal internal) {
        System.out.println(internal+"==="+healthyDao.getInternalList(internal));
        return healthyDao.getInternalList(internal);
    }

    @Override
    public String insertInternal(Internal internal) {
        int i = healthyDao.insertInternalOnly(internal);
        if (i>0){
            return "success";
        }
        return "error";
